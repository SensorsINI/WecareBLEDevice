...
//		spi2_adc_write_register(CONFIG2, sendBuf, CONFIG2_BYTES);	
		...
		// Configure SCAN register: 4 Differential Channels.
		sendBuf[0] = 0x00;
		sendBuf[1] = 0x0F;
		sendBuf[2] = 0x00;
...
/**
 ****************************************************************************************
 * @brief SPI2 ADC2 module (MCP3564R) init function.
 * @return void
 ****************************************************************************************
*/
void spi2_adc2_init(void)
{	
	  uint32_t regVal;
	  uint8_t sendBuf[TOTAL_BYTES] = {0};
	  uint8_t receiveBuf[TOTAL_BYTES] = {0}; 
...
//		spi2_adc_write_register(CONFIG2, sendBuf, CONFIG2_BYTES);	
		...
		// Configure SCAN register: 8 Single-Ended Channels.
		sendBuf[0] = 0x00;
		sendBuf[1] = 0x00;
		sendBuf[2] = 0xFF;
		spi2_adc_write_register(SCAN, sendBuf, SCAN_BYTES);
...
//		GLOBAL_INT_STOP();	
    ...
    // Obtain the GAIN bits value
	  ...
	  // Convert it to the real gain
		...
		memcpy(ADCReadValBuf, voltage, sizeof(float) * 4);			
 ...
 ****************************************************************************************
*/
void spi2_adc2_readout(uint32_t *ADCReadValBuf)
{ 
...
//		GLOBAL_INT_STOP();	
    ...
    // Obtain the GAIN bits value
	  ...
	  // Convert it to the real gain
		...
		// Copy voltage hex buffer to value shared with BLE for sending to the host
		memcpy(ADCReadValBuf, voltage, sizeof(float) * 8);			
    ADCReadValBuf[12] = adc2ConversionCount; // Store the ADC conversion count
		ADCReadValBuf[13] = errorCntADC2;
		ADCReadValBuf[14] = validFlg;           // Indicate the current conversion valid or not
